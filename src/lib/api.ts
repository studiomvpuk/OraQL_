...
class ApiClient {
  ...
  setToken(token: string | null) {
    ...
    if (token) {
      if (typeof window !== 'undefined') localStorage.setItem('oracle_token', token);
    } else {
      if (typeof window !== 'undefined') localStorage.removeItem('oracle_token');
    }
  }

  getToken(): string | null {
    if (this.accessToken) return this.accessToken;
    if (typeof window !== 'undefined') return localStorage.getItem('oracle_token');
    return null;
  }
  ...
  private async request<T>(method: string, path: string, body?: unknown, options: RequestInit = {}): Promise<T> {
    ...
    const response = await fetch(url, { method, headers, body: body ? JSON.stringify(body) : undefined, ...options });
    if (!response.ok) {
